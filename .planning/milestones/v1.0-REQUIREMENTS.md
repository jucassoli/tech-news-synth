# tech-news-synth — v1.0 Requirements (ARCHIVED)

**Milestone:** v1.0 (initial production-ready agent on @ByteRelevant)
**Status:** ✅ ALL 53 REQUIREMENTS COMPLETE — SHIPPED 2026-04-14
**Outcome:** All requirements delivered as specified; operator sign-off on Phase 8 Parts A + B + C closed v1.0 acceptance.
**Derived from:** `.planning/PROJECT.md`, `.planning/research/SUMMARY.md` + research files
**Last updated:** 2026-04-14 (archive)
...
- [x] **GATE-01** — Smoke test script confirms Anthropic API access with `claude-haiku-4-5` returns a valid completion on a minimal prompt
- [x] **GATE-02** — Smoke test script confirms X OAuth 1.0a User Context: `client.get_me()` succeeds with the 4 configured secrets
- [x] **GATE-03** — Smoke test script publishes one real tweet to @ByteRelevant and deletes it; records actual cost per post and daily cap observed from X response headers
- [x] **GATE-04** — Gate outcome (cost model, caps, OAuth permissions state) is documented in `.planning/intel/x-api-baseline.md` before pipeline work proceeds

### STORE — Persistence Layer

- [x] **STORE-01** — Postgres schema managed by Alembic migrations, versioned in the repo; `alembic upgrade head` runs on container startup
...
- [x] **STORE-03** — `clusters` table persists cluster metadata per cycle (cycle_id, member article ids, centroid/top-K terms, chosen status, coverage score)
...
- [x] **STORE-05** — `run_log` table records every cycle: `cycle_id`, `started_at`, `finished_at`, `status`, `counts` (articles fetched per source, clusters formed), `notes`
...
- [x] **INGEST-01** — Source list loaded from a mounted `sources.yaml` (not DB); schema supports add/edit/remove without code changes; invalid entries fail boot with clear error
...
- [x] **INGEST-03** — Each source fetch uses `httpx` with a per-source timeout, descriptive User-Agent `ByteRelevant/0.1 (+https://x.com/ByteRelevant)`, and `tenacity` retry (max 3, exponential backoff)
...
- [x] **INGEST-06** — Normalizer produces a unified `Article` dataclass: `id`, `source`, `url`, `canonical_url`, `title`, `summary`, `published_at` (UTC aware), `fetched_at`, `article_hash`; HTML stripped from summary via `beautifulsoup4/lxml`
...
- [x] **CLUSTER-01** — Clustering operates on articles from the last `CLUSTER_WINDOW_HOURS` (default 6) across all active sources
...
- [x] **CLUSTER-05** — Anti-repetition filter: before accepting a winner, compute cosine similarity against centroids of posts in the last 48h; reject if any ≥ 0.5 and fall back to the next best cluster
...
- [x] **CLUSTER-07** — Cluster-selection audit trail persisted per cycle: which cluster won, why (scores), which were rejected by anti-repeat, for operator inspection
...
- [x] **SYNTH-04** — Character budget enforced in Python after the LLM response using a weighted char count (t.co URL fixed at 23, plus hashtag budget); max 2 re-prompt retries with "encurte para N caracteres" before last-resort whitespace-aware truncation with ellipsis
...
- [x] **SYNTH-07** — Token usage and USD cost per synthesis call are logged in the cycle summary and stored on the `posts` row
...
- [x] **PUBLISH-02** — Idempotent posting: a `posts` row with `status=pending` is inserted **before** the X API call; updated to `posted` + `tweet_id` on success, or `failed` with error details on error
- [x] **PUBLISH-03** — Rate-limit handling: 429 response reads `x-rate-limit-reset`, logs a structured warning, and skips the remainder of the cycle (does not block the scheduler loop)
...
- [x] **PUBLISH-06** — On dry-run, publisher is short-circuited; the `posts` row is written with `status=dry_run` and the synthesized text for human review
...
- [x] **OPS-05** — `docker compose up -d` on a fresh Ubuntu VPS produces a working agent within a documented runbook (`docs/DEPLOY.md`)
- [x] **OPS-06** — 48-hour soak in `DRY_RUN=1` mode passes with zero unhandled exceptions and at least one cycle per 2h window before live cutover
