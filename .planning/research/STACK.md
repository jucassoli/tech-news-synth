...
**Domain:** Automated tech-news curation + NLP clustering + LLM synthesis + X posting agent (Python, Docker Compose, Ubuntu VPS)
**Researched:** 2026-04-12
**Confidence:** HIGH (all versions verified against official docs / PyPI / GitHub as of April 2026)
...
> **Deliberately NOT included:** `requests`, `aiohttp`, `loguru`, `psycopg2-binary`, `python-dotenv` (standalone), `schedule` library, `celery`, `redis`, embedding libraries (`sentence-transformers`, `openai` embeddings), `black`, `flake8`, `isort`, `mypy`-as-primary. See "What NOT to Use" below.
...
**`pyproject.toml` dependency block (recommended with `uv`):**
...
**Dockerfile (multi-stage, slim, uv):**
...
CMD ["python", "-m", "tech_news_synth.scheduler"]
```

Use **`python:3.12-slim-bookworm`** (Debian 12). Avoid Alpine for Python (musl libc breaks wheels, triggers source compilation of scikit-learn/lxml → 10× slower builds).
...
**Rate-limit / retry pattern (X Free tier = ~17 posts/day):**
...
**OAuth 1.0a User Context setup (one-time):**
...
**Cluster hash pattern (48h anti-repeat):**
...
**If volume grows (>50 posts/day future):**
...
**If you need observability later (post-MVP):**
...
*Stack research for: Python tech-news curation + X posting agent*
*Researched: 2026-04-12*
*All versions verified against official PyPI / docs as of this date.*
